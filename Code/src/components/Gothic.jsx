...
import { gothic } from '../data/questions.json';
import $ from 'jquery';

class Gothic extends Component {
    state = {
        points: 0,
        nr: 0,
        dialogue: ''
            ...
            <React.Fragment>
                <div key="quiz" className="quiz">
                    <div key='1' className="quiz-question">
                        ...
                        <p>{gothic.questions[this.state.nr]}</p>
                    </div>
                    <div key='2' className="quiz-answers">
                        {gothic.answers[this.state.nr].map(m => 
                            <p key={m[0]} onClick={() => this.handleShowNextQuestion(m[1])}>{m[0]}</p>
                        )}
                    </div>
                </div>
                <div className="result hidden">
                    <img src="./images/title.png" alt="Tytuł gry"/>
