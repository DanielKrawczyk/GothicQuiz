...
import { gothic2 } from '../data/questions.json';
import $ from 'jquery';

class Gothic2 extends Component {
    state = {
        points: 0,
        nr: 0,
        dialogue: ''
            ...
            <React.Fragment>
                <div key="quiz" className="quiz">
                    <div key='1' className="quiz-question">
                        ...
                        <p>{gothic2.questions[this.state.nr]}</p>
                    </div>
                    <div key='2' className="quiz-answers">
                        {gothic2.answers[0][this.state.nr].map(m => 
                            <p key={m[0]} onClick={() => this.handleShowNextQuestion(m[1])}>{m[0]}</p>
                        )}
                    </div>
                </div>
                <div className="result hidden">
                    <img src="./images/gothic2.png" alt="Tytuł gry"/>
